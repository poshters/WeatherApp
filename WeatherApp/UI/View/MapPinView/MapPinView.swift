...
final class MapPinView: UIView {
    ...
    /// Draw header UIBezierPath
    private func drawHeader() {
        let start: CGPoint = CGPoint(x: 20, y: 0)
        let line1 = CGPoint(x: bounds.width - 20, y: 0)
        let line2 = CGPoint(x: bounds.width, y: 20)
        let line3 = CGPoint(x: bounds.width, y: self.bounds.height - 40)
        let line4 = CGPoint(x: bounds.width - 20, y: self.bounds.height - 20)
        let line5 = CGPoint(x: bounds.width / 1.8, y: self.bounds.height - 20)
        let line6 = CGPoint(x: bounds.width / 2, y: self.bounds.height)
        let line7 = CGPoint(x: bounds.width / 2.2, y: self.bounds.height - 20)
        let line8 = CGPoint(x: 20, y: bounds.height - 20)
        let line9 = CGPoint(x: 0, y: bounds.height - 40)
